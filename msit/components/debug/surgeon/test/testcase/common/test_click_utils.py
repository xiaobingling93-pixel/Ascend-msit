...
class TestCheckInputPath(unittest.TestCase):
    ...
    def test_check_input_path_given_existing_readable_path_when_checked_then_return_true(self, mock_logger, mock_access):
    ...
    @patch('os.access')
    @patch('components.debug.common.logger.error')
    def test_check_input_path_given_existing_non_readable_path_when_checked_then_return_false(self, mock_logger, mock_access):
...
class TestCheckOutputModelPath(unittest.TestCase):
    ...
    @patch('components.debug.common.logger.error')
    def test_check_output_model_path_given_valid_path_when_checked_then_return_true(self, mock_logger, mock_exists, mock_isdir):
    ...
    @patch('os.path.exists')
    @patch('components.debug.common.logger.error')
    def test_check_output_model_path_given_directory_path_when_checked_then_return_false(self, mock_logger, mock_exists, mock_isdir):
    ...
    @patch('os.path.exists')
    @patch('components.debug.common.logger.error')
    def test_check_output_model_path_given_non_existing_path_when_checked_then_return_false(self, mock_logger, mock_exists, mock_isdir):
        mock_isdir.return_value = False
...
class TestOptimizeOnnx(unittest.TestCase):
    ...
    @patch('auto_optimizer.graph_refactor.onnx.graph.OnnxGraph.parse')
    @patch('components.debug.common.logger.warning')
    def test_optimize_onnx_given_valid_model_when_optimized_then_return_applied_knowledges(self, mock_logger, mock_parse):
        optimizer = MagicMock(spec=GraphOptimizer)
        optimizer.apply_knowledges.return_value = (MagicMock(), ['Knowledge1', 'Knowledge2'])
        input_model = pathlib.Path("valid_model.onnx")
        output_model = pathlib.Path("output_model.onnx")
        self.assertEqual(optimize_onnx(optimizer, input_model, output_model, False, InferTestConfig()), ['Knowledge1', 'Knowledge2'])
        mock_logger.assert_not_called()
...
class TestEvaluateOnnx(unittest.TestCase):
    ...
    @patch('auto_optimizer.graph_refactor.onnx.graph.OnnxGraph.parse')
    @patch('components.debug.common.logger.warning')
    def test_evaluate_onnx_given_valid_model_when_evaluated_then_return_applied_knowledges(self, mock_logger, mock_parse):
        optimizer = MagicMock(spec=GraphOptimizer)
        optimizer.apply_knowledges.return_value = (MagicMock(), ['Knowledge1', 'Knowledge2'])
