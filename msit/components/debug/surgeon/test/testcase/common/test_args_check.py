...
class TestPathLegalityFunctions(unittest.TestCase):
    ...
    def test_check_soc_digit_string_valid_id(self, mock_subprocess_run, mock_glob):
        mock_glob.return_value = ["/dev/davinci0", "/dev/davinci1"]
        mock_subprocess_run.return_value.stdout = b"dev0 davinci0 dev1 davinci1"
    ...
    @patch('glob.glob')
    @patch('subprocess.run')
    def test_check_soc_digit_string_invalid_id(self, mock_subprocess_run, mock_glob):
        mock_glob.return_value = ["/dev/davinci0"]
        mock_subprocess_run.return_value.stdout = b"dev0 davinci0"
        mock_subprocess_run.return_value.returncode = 0

        with pytest.raises(ArgumentTypeError) as excinfo:
            check_soc("2")
        assert str(excinfo.value) == "2 is not a valid value. Please check device id. "

    @patch('glob.glob')
    @patch('subprocess.run')
    def test_check_soc_digit_string_single_device(self, mock_subprocess_run, mock_glob):
        mock_glob.return_value = ["/dev/davinci0"]
        mock_subprocess_run.return_value.stdout = b"dev0 davinci0"
        mock_subprocess_run.return_value.returncode = 0
        ...
        with pytest.raises(ArgumentTypeError) as excinfo:
            check_soc("1")
        assert str(excinfo.value) == "1 is not a valid value. Please check device id. "
