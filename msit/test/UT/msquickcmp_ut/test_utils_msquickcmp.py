...
from components.debug.compare.msquickcmp.common.utils import (
    ...
    check_file_size_valid, check_input_args, check_convert_is_valid_used,
    check_locat_is_valid, check_device_param_valid,
    ...
    ACCURACY_COMPARISON_INVALID_PARAM_ERROR,
    ACCURACY_COMPARISON_INVALID_COMMAND_ERROR,
    ACCURACY_COMPARISON_INVALID_DEVICE_ERROR,
    INVALID_CHARS,
    MAX_DEVICE_ID,
    _check_colon_exist,
...
class TestCheckFunctions1(unittest.TestCase):
    ...
    @patch('components.debug.compare.msquickcmp.common.utils.logger')
    @patch('components.debug.compare.msquickcmp.common.utils.os.path.exists')
    @patch('components.debug.compare.msquickcmp.common.utils.os.access')
    ...
    @patch('components.debug.compare.msquickcmp.common.utils.logger')
    @patch('components.debug.compare.msquickcmp.common.utils.os.path.isdir')
    @patch('components.debug.compare.msquickcmp.common.utils.os.access')
    ...
    @patch('components.debug.compare.msquickcmp.common.utils.logger')
    @patch('components.debug.compare.msquickcmp.common.utils.os.path.isfile')
    @patch('components.debug.compare.msquickcmp.common.utils.os.access')
    ...
    @patch('components.debug.compare.msquickcmp.common.utils.check_file_or_directory_path')
    @patch('components.debug.compare.msquickcmp.common.utils.get_input_path')
    @patch('components.debug.compare.msquickcmp.common.utils.os.path.realpath')
    ...
    @patch('components.debug.compare.msquickcmp.common.utils.logger')
    @patch('components.debug.compare.msquickcmp.common.utils.os.stat')
...
class TestCheckFunctions2(unittest.TestCase):
    ...
    @patch('components.debug.compare.msquickcmp.common.utils.logger')
    @patch('components.debug.compare.msquickcmp.common.utils.get_shape_not_match_message')
    ...
    def test_check_shape_number_valid(self):
        valid_shape = "123"
        with patch('components.debug.compare.msquickcmp.common.utils.DIM_PATTERN', r'^\d+$'):
    ...
    @patch('components.debug.compare.msquickcmp.common.utils.os.walk')
...
class TestYourFunctions(unittest.TestCase):
    @patch("components.debug.compare.msquickcmp.common.utils.os")
    @patch("components.debug.compare.msquickcmp.common.utils.shutil")
    @patch("components.debug.compare.msquickcmp.common.utils.logger")
    ...
    @patch("components.debug.compare.msquickcmp.common.utils.os")
    @patch("components.debug.compare.msquickcmp.common.utils.get_batch_index_from_name")
    ...
    @patch("components.debug.compare.msquickcmp.common.utils.get_batch_index")
...
class TestParseInputShapeToList(unittest.TestCase):
    @patch('components.debug.compare.msquickcmp.common.utils._check_colon_exist')
    @patch('components.debug.compare.msquickcmp.common.utils.dym_shape_range_interaction', return_value=True)
    def test_valid_shape(self, mock_interact, mock_check):
        from components.debug.compare.msquickcmp.common.utils import parse_input_shape_to_list
    ...
    def test_invalid_shape_format(self, mock_check):
        from components.debug.compare.msquickcmp.common.utils import parse_input_shape_to_list, AccuracyCompareException
        with patch('components.debug.compare.msquickcmp.common.utils.logger.error'), \
             patch('components.debug.compare.msquickcmp.common.utils.get_shape_not_match_message', return_value="error"):
            with self.assertRaises(AccuracyCompareException):
...
class TestDymShapeRangeInteraction(unittest.TestCase):
    ...
    def test_yes_input(self, mock_input):
        from components.debug.compare.msquickcmp.common.utils import dym_shape_range_interaction
...
class TestParseDymShapeRange(unittest.TestCase):
    @patch('components.debug.compare.msquickcmp.common.utils._check_colon_exist')
    @patch('components.debug.compare.msquickcmp.common.utils._check_shape_number')
    @patch('components.debug.compare.msquickcmp.common.utils._check_content_split_length')
    @patch('components.debug.compare.msquickcmp.common.utils.dym_shape_range_interaction', return_value=True)
    @patch('components.debug.compare.msquickcmp.common.utils.logger.info')
    def test_parse_valid_range(self, mock_log, mock_interact, mock_len, mock_shape, mock_colon):
        from components.debug.compare.msquickcmp.common.utils import parse_dym_shape_range
        result = parse_dym_shape_range("tensor1:1,2~3")
        self.assertIsInstance(result, list)


class TestParseArgValue(unittest.TestCase):
    @patch('components.debug.compare.msquickcmp.common.utils.parse_value_by_comma')
    def test_parse(self, mock_parse):
        from components.debug.compare.msquickcmp.common.utils import parse_arg_value
...
class TestParseValueByComma(unittest.TestCase):
    def test_valid_values(self):
        from components.debug.compare.msquickcmp.common.utils import parse_value_by_comma
        result = parse_value_by_comma("1,2,3")
        self.assertEqual(result, [1, 2, 3])

    def test_invalid_values(self):
        from components.debug.compare.msquickcmp.common.utils import parse_value_by_comma, AccuracyCompareException
        with patch('components.debug.compare.msquickcmp.common.utils.logger.error'):
            with self.assertRaises(AccuracyCompareException):
...
class TestExecuteCommand(unittest.TestCase):
    @patch('components.debug.compare.msquickcmp.common.utils.subprocess.Popen')
    @patch('components.debug.compare.msquickcmp.common.utils.filter_cmd', return_value=['echo', 'test'])
    def test_command_success(self, mock_filter, mock_popen):
        from components.debug.compare.msquickcmp.common.utils import execute_command
        mock_proc = MagicMock()
        mock_proc.poll.side_effect = [None, 0]
        mock_proc.stdout.readline.side_effect = [b'output\n', b'']
        mock_proc.returncode = 0
        mock_popen.return_value = mock_proc
        with patch('components.debug.compare.msquickcmp.common.utils.logger.info'):
            execute_command(['echo', 'test'])

    @patch('components.debug.compare.msquickcmp.common.utils.subprocess.Popen')
    @patch('components.debug.compare.msquickcmp.common.utils.filter_cmd', return_value=['bad', 'cmd'])
    def test_command_fail(self, mock_filter, mock_popen):
        from components.debug.compare.msquickcmp.common.utils import execute_command, AccuracyCompareException
        mock_proc = MagicMock()
        mock_proc.poll.side_effect = [None, 0]
        mock_proc.stdout.readline.return_value = b'error\n'
...
class TestCreateDirectory(unittest.TestCase):
    @patch('components.debug.compare.msquickcmp.common.utils.os.path.exists', return_value=False)
    @patch('components.debug.compare.msquickcmp.common.utils.ms_makedirs')
    def test_create_success(self, mock_mkdirs, mock_exists):
        from components.debug.compare.msquickcmp.common.utils import create_directory
        create_directory('/tmp/fake')

    @patch('components.debug.compare.msquickcmp.common.utils.os.path.exists', return_value=False)
    @patch('components.debug.compare.msquickcmp.common.utils.ms_makedirs', side_effect=OSError("Permission denied"))
    def test_create_fail(self, mock_mkdirs, mock_exists):
        from components.debug.compare.msquickcmp.common.utils import create_directory, AccuracyCompareException
        with patch('components.debug.compare.msquickcmp.common.utils.logger.error'):
            with self.assertRaises(AccuracyCompareException):
                create_directory('/tmp/fake')
...
class TestSaveNumpyData(unittest.TestCase):
    @patch('components.debug.compare.msquickcmp.common.utils.os.path.exists', return_value=False)
    @patch('components.debug.compare.msquickcmp.common.utils.ms_makedirs')
    @patch('components.debug.compare.msquickcmp.common.utils.np.save')
    def test_save(self, mock_save, mock_mkdirs, mock_exists):
        from components.debug.compare.msquickcmp.common.utils import save_numpy_data
        save_numpy_data('/tmp/data.npy', [1, 2, 3])


class TestHandleGroundTruthFiles(unittest.TestCase):
    @patch('components.debug.compare.msquickcmp.common.utils.get_batch_index', return_value=1)
    @patch('components.debug.compare.msquickcmp.common.utils.shutil.copy')
    @patch('components.debug.compare.msquickcmp.common.utils.os.walk')
    def test_handle(self, mock_walk, mock_copy, mock_batch):
        from components.debug.compare.msquickcmp.common.utils import handle_ground_truth_files
        parser_mock = MagicMock()
        parser_mock.get_dynamic_scenario_info.return_value = (None, 'DYM_BATCH')
        mock_walk.return_value = [('/root', [], ['x.bin'])]
        with patch('components.debug.compare.msquickcmp.common.utils.BATCH_SCENARIO_OP_NAME', '{}_{}'):
            handle_ground_truth_files(parser_mock, '/npu', '/golden')


class TestStr2Bool(unittest.TestCase):
    def test_valid_true(self):
        from components.debug.compare.msquickcmp.common.utils import str2bool
...
class TestMergeCSV(unittest.TestCase):
    @patch('components.debug.compare.msquickcmp.common.utils.load_file_to_read_common_check')
    @patch('components.debug.compare.msquickcmp.common.utils.pd.read_csv')
    @patch('components.debug.compare.msquickcmp.common.utils.pd.concat')
    def test_merge(self, mock_concat, mock_read, mock_load):
        from components.debug.compare.msquickcmp.common.utils import merge_csv
        df_mock = MagicMock()
...
class TestSafeDeletePathIfExists(unittest.TestCase):
    @patch("components.debug.compare.msquickcmp.common.utils.get_valid_write_path")
    @patch("components.debug.compare.msquickcmp.common.utils.logger")
    ...
    @patch("os.remove")
    @patch("shutil.rmtree")
    def test_delete_file(self, mock_rmtree, mock_remove, mock_isfile, mock_isdir, mock_exists, mock_logger, mock_get_valid_path):
        mock_exists.return_value = True
        mock_isdir.return_value = False
        mock_isfile.return_value = True
    ...
    @patch("os.remove")
    @patch("shutil.rmtree")
    def test_delete_folder(self, mock_rmtree, mock_remove, mock_isfile, mock_isdir, mock_exists, mock_logger, mock_get_valid_path):
        mock_exists.return_value = True
        mock_isdir.return_value = True
        mock_isfile.return_value = False
...
class TestParseJsonFile(unittest.TestCase):
    @patch("components.debug.compare.msquickcmp.common.utils.load_file_to_read_common_check")
    @patch("components.debug.compare.msquickcmp.common.utils.ms_open", read_data='{"key": "value"}')
    @patch("json.load")
    ...
    @patch("components.debug.compare.msquickcmp.common.utils.load_file_to_read_common_check")
    @patch("components.debug.compare.msquickcmp.common.utils.ms_open", side_effect=FileNotFoundError("No such file"))
    def test_parse_json_file_not_found(self, mock_open_file, mock_load_check):
        mock_load_check.return_value = "missing.json"
        with self.assertRaises(FileNotFoundError) as context:
            parse_json_file("missing.json")
        self.assertIn("missing.json", str(context.exception))

    @patch("components.debug.compare.msquickcmp.common.utils.load_file_to_read_common_check")
    @patch("components.debug.compare.msquickcmp.common.utils.ms_open")
    @patch("json.load", side_effect=json.JSONDecodeError("Expecting value", "doc", 0))
    def test_parse_json_decode_error(self, mock_json_load, mock_open_file, mock_load_check):
        mock_load_check.return_value = "bad.json"
        with self.assertRaises(RuntimeError) as context:
            parse_json_file("bad.json")
