...
class TestKnowledgeMergeCasts(unittest.TestCase, KnowledgeTestHelper):
    ...
    def test_merge_brother_casts(self, mock_parse, mock_file, mock_inference_session):
    ...
    @patch('builtins.open', new_callable=mock_open)
    @patch('auto_optimizer.graph_refactor.onnx.graph.OnnxGraph.parse')
    def test_transfer_cast_to_root(self, mock_parse, mock_file, mock_inference_session):
        ...
        self.assertEqual(len(next_nodes), 2)
        self.assertEqual(next_nodes[0].op_type, 'Cast')

    @patch('onnxruntime.InferenceSession')
    @patch('builtins.open', new_callable=mock_open)
    @patch('auto_optimizer.graph_refactor.onnx.graph.OnnxGraph.parse')
    def test_remove_parent_cast(self, mock_parse, mock_file, mock_inference_session):
        ...
        self.assertEqual(next_nodes[0].name, 'Cast1')

    @patch('onnxruntime.InferenceSession')
    @patch('builtins.open', new_callable=mock_open)
    @patch('auto_optimizer.graph_refactor.onnx.graph.OnnxGraph.parse')
    def test_remove_cast_after_root(self, mock_parse, mock_file, mock_inference_session):
