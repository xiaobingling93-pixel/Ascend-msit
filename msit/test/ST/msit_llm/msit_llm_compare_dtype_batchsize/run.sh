...
msit llm compare -gp $MODEL_PATH/tensors_0/ -mp $MODEL_PATH/tensors_1/  #冒烟用例脚本，需根据自己的用例需要修改
if [$? -eq 0 ]
then
    echo msit_llm_compare_dtype_batchsize: Success
else
    echo msit_llm_compare_dtype_batchsize: Failed
    run_ok=$ret_failed
fi
