...
echo -e "\033[1;32m[1/1]\033[0m msit_llm_compare_torch_atb_encode_decode测试用例"
...
echo msit llm compare \
    --golden-path $MODEL_PATH/compare_data/encode-decode/gpu_dump/cuda0_66245/0 \
    --my-path $MODEL_PATH/compare_data/encode-decode/msit_dump/tensors/0_1445461/0 \
     -o $OUTPUT_PATH  #冒烟用例脚本，需根据自己的用例需要修改
...
if [ $? -eq 0 ]
then
    echo msit_llm_compare_torch_atb_encode_decode测试用例: Success
else
    echo msit_llm_compare_torch_atb_encode_decode测试用例: Failed
    run_ok=$ret_failed
fi
