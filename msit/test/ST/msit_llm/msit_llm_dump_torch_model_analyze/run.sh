...
echo -e "\033[1;32m[1/1]\033[0m msit_llm_dump_torch_model_analyze 测试用例"
...
rm ait_dump_torch -rf
mkdir ait_dump_torch
echo `realpath ait_dump_torch`
...
if [ $? -eq 0 ]
then
    echo msit_llm_dump_torch_model_analyze: Success
else
    echo msit_llm_dump_torch_model_analyze: Failed
    run_ok=$ret_failed
fi
