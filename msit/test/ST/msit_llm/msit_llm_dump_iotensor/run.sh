...
echo -e "\033[1;32m[1/1]\033[0m msit_llm_dump_iotensor测试用例"
...
msit llm dump --exec "bash $MODEL_PATH/mindie_atb_models/examples/models/chatglm/v2_6b/run_300i_duo_pa.sh $MODEL_PATH/chatglm2_6b" -sd -o $OUTPUT_PATH --type op kernel #冒烟用例脚本，需根据自己的用例需要修改
if [ $? -eq 0 ]
then
    echo msit_llm_dump_iotensor: Success
else
    echo msit_llm_dump_iotensor: Failed
    run_ok=$ret_failed
fi
