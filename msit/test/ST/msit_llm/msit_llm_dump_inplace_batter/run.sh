...
echo -e "\033[1;32m[1/1]\033[0m msit_llm_dump_inplace_batter 测试用例"
...
source /usr/local/Ascend/latest/ascend-toolkit/set_env.sh
source $MODEL_PATH/atb/set_env.sh
source $MODEL_PATH/mindie_atb_models/set_env.sh
unset ATB_SAVE_TENSOR_RUNNER

cd $MODEL_PATH/mindie_atb_models/examples
msit llm dump --exec "bash $MODEL_PATH/mindie_atb_models/examples/models/chatglm/v2_6b/run_300i_duo_pa.sh $MODEL_PATH/chatglm2_6b" 
if [ $? -eq 0 ]
then
    echo msit_llm_dump_inplace_batter: Success
else
    echo msit_llm_dump_inplace_batter: Failed
    run_ok=$ret_failed
fi
