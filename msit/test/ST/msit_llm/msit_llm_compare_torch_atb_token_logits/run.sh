...
echo -e "\033[1;32m[1/1]\033[0m msit_llm_compare_torch_atb_token_logits测试用例"
...
echo msit llm compare \
    --golden-path $MODEL_PATH/compare_data/token_logits/gpu_dump/cuda0_66245 \
    --my-path $MODEL_PATH/compare_data/token_logits/msit_dump/tensors/0_1445461 \
    -cl token \
...
if [ $? -eq 0 ]
then
    echo msit_llm_compare_torch_atb_token_logits测试用例: Success
else
    echo msit_llm_compare_torch_atb_token_logits测试用例: Failed
    run_ok=$ret_failed
fi
