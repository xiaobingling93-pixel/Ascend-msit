...
echo -e "\033[1;32m[1/1]\033[0m msit_llm_dump_manual_mapping_comparison测试用例"
...
echo y | msit llm compare -gp $MODEL_PATH/tensors_4 -mp $MODEL_PATH/outtensor0.bin -o $OUTPUT_PATH #冒烟用例脚本，需根据自己的用例需要修改
if [ $? -eq 0 ]
then
    echo msit_llm_dump_manual_mapping_comparison: Success
else
    echo msit_llm_dump_manual_mapping_comparison: Failed
    run_ok=$ret_failed
fi
