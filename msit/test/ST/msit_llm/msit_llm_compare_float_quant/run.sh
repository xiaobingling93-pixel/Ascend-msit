...
echo -e "\033[1;32m[1/1]\033[0m msit_llm_compare_float_quant测试用例"
...
msit llm compare --golden-path $MODEL_PATH/tensors_6 --my-path $MODEL_PATH/tensors_7 -o $OUTPUT_PATH  #冒烟用例脚本，需根据自己的用例需要修改
if [ $? -eq 0 ]
then
    echo msit_llm_compare_float_quant: Success
else
    echo msit_llm_compare_float_quant: Failed
    run_ok=$ret_failed
fi
