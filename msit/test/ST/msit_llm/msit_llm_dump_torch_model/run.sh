...
echo -e "\033[1;32m[1/1]\033[0m msit_llm_dump_torch_model测试用例"
...
python main_torch.py --mode precision_single --model_path $MODEL_PATH/ascend-speed-inference/pytorch/examples/chatglm2_6b --batch 1 --model_file patches/modeling_chatglm_npu.py
if [ $? -eq 0 ]
then
    echo msit_llm_dump_torch_model: Success
else
    echo msit_llm_dump_torch_model: Failed
    run_ok=$ret_failed
fi
