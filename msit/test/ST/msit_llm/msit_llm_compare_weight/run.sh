...
echo -e "\033[1;32m[1/1]\033[0m msit_llm_compare_dtype_batchsize测试用例"
...
echo "y" | msit llm compare --weight -gp $MODEL_PATH/msit_llm_compare_weight/float/ -mp $MODEL_PATH/msit_llm_compare_weight/quanted/ #冒烟用例脚本，需根据自己的用例需要修改
if [ $? -eq 0 ]
then
    echo msit_llm_compare_weight: Success
else
    echo msit_llm_compare_weight: Failed
    run_ok=$ret_failed
fi
