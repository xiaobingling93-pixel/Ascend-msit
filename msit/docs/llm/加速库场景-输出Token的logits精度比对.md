...
使用[加速库 Dump 功能](/msit/docs/llm/工具-DUMP加速库数据使用说明.md)。需要 Dump 两次：
...
> - layer 信息落盘位置：在`{DUMP_DIR}/ait_dump_{TIMESTAMP}/layer/{PID}/`目录下，可以通过 layer 信息，获取到最大的编号，编号最大的即是模型最后一层
...
## 第三步：将上面两个步骤生成的 dump 文件，输入到 msit llm compare 中完成比对

使用[自动比对功能](/msit/docs/llm/工具-自动比对功能使用说明.md)。比对标杆数据和加速库数据。

### 使用方式

```shell
msit llm compare -gp {DUMP_DIR}/{cuda|cpu}{device_id}_{PID}/ -mp ait_dump_{TIMESTAMP}/tensors/{device_id}_{PID}/ -cl token
```

msit llm compare 提供有精度问题的数据与标杆数据之间的比对能力。

> - --golden-path 参数为第一步中 `torch_tensor`所在目录 `{DUMP_DIR}/{PID}_npu{device_id}/`
> - --my-path 参数为第二步中 `atb_tensor`所在目录 `{DUMP_DIR}/ait_dump_{TIMESTAMP}/tensors/{device_id}_{PID}/`
> - -cl token 参数指定比对的 token 数据。
