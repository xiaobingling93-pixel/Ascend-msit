...
opcheck提供加速库内置算子的单算子精度预检能力，检测加速库算子精度是否达标。若使用旧版msit llm，请参考[精度预检能力使用说明](/msit/docs/llm/v1.0/精度预检能力使用说明.md)。

执行步骤：
1. 使用`msit llm dump`落盘模型推理真实数据
2. 执行`msit llm opcheck`，预检工具将根据真实数据自动构造算子精度检测用例，并计算标杆output
...
- 从源码安装 msit_llm 时，如果配置了 `atb` 以及 `mindie-atb-models` 执行环境，会自动编译安装 `libopchecker.so`，不需要再手动编译
- 从 whl 包安装 msit_llm 时或安装时未配置 `atb` 以及 `mindie-atb-models` 执行环境，需要手动编译 `libopchecker.so`
  - 执行编译命令
    ```
...
## 2. 输入数据
### 2.1 数据落盘
使用`msit llm dump --exec "bash run.sh patches/models/modeling_xxx.py"`将模型推理过程中的tensor数据落盘。
- `--exec`参数用于指定拉起执行模型推理脚本的命令。**用户需自行保证命令的安全性，并承担因输入不当而导致的任何安全风险或损失**
- `msit llm dump`相关参数使用请参考[加速库DUMP功能使用说明](/msit/docs/llm/工具-DUMP加速库数据使用说明.md)
- tensor信息会生成在落盘路径的msit_dump目录下，具体路径是 `{DUMP_DIR}/msit_dump_{TIMESTAMP}/tensors/{device_id}_{PID}/{TID}`目录下。
...
### 3.1 使用示例
```bash
msit llm opcheck -i {OUTPUT_DIR}/msit_dump_{TIMESTAMP}/tensors/{device_id}_{PID}/{TID}/
```
#### 参数说明

| 参数名                      | 描述                                                         | 是否必选 |
| --------------------------- | ------------------------------------------------------------ | -------- |
| --input, -i                 | tensor数据路径，为文件夹，由msit llm dump --type tensor落盘，示例：{OUTPUT_DIR}/msit_dump_{TIMESTAMP}/tensors/{device_id}_{PID}/{TID}/ | 是       |
...
| --precision-mode, -pmode  | 指定golden计算及比对精度，可选范围：["keep_origin_dtype", "force_fp16", "force_fp32"]，分别表示保持原精度、浮点数转成float16、浮点数转成float32计算golden并比对。默认为保持原精度计算并比对。使用方式：-pmode force_fp16 | 否       |
| --log-level, -l            | 日志级别，默认为info，可选值有：debug, info, warning, error, fatal, critical | 否       |
| --jobs, -j                 | 指定进程数，默认为1，即单进程拉起精度预检。可选范围为1~8个进程数。注意该参数在与--atb-rerun/-rerun同时使用时会被忽略，rerun模式只支持单进程拉起精度预检 | 否       |
| --optimization-identify, -opt                 | 识别FA/PA算子各优化项对算子精度的影响。此参数必须与--atb-rerun/-rerun同时使用时才会生效。 | 否       |
| --csv-path, -c              | （新版本弃用）算子信息csv文件路径，为单个数据文件路径，由msit llm dump --type op落盘，示例：OUTPUT_DIR/msit_dump_{TIMESTAMP}/operation_io_tensors/PID/operation_tensors_0.csv | 是       |
| --help, -h              | 命令行参数帮助信息| 否       |

注：
- `{OUTPUT_DIR}/msit_dump_{TIMESTAMP}/tensors/{device_id}_{PID}/{TID}`为tensor数据根目录，`-i`参数可接受根目录下任意子目录
...
| 算子名称                          | Atlas 300I Duo  | Atlas 800I A2 |
|-------------------------------| --------- | ---------- |
...
| MatmulOperation（加速库已删除，后续不维护）| 支持 | 支持 |
...
2. 为什么在Atlas 300I Duo上部分算子精度预检比对结果不可靠？

如果输入intensors使用after数据（即保存时间节点为算子执行后的输入数据），部分算子在Atlas 300I Duo上精度预检比对结果不可靠。这是由于Atlas 300I Duo上部分算子（如linearOperation、PagedAttentionOperationd等）存在原地读写的情况，导致部分输入数据成为“脏数据”，从而影响比对结果。这种情况推荐使用`-rerun`参数重新运行算子，使用同一套输入数据计算真实output和标杆output，再进行比对。
