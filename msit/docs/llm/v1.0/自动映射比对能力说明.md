...
* dump完成后tensor信息生成在`{DUMP_DIR}/msit_dump/tensors/{device_id}_{PID}/{TID}`目录下
* dump完成后model拓扑信息生成在`{DUMP_DIR}/msit_dump/model/{PID}`目录下。由于model由layer组合而成，因此使用model时，默认同时会落盘layer信息。
...
`msit llm compare -gp torch_dump/{PID}_npu{device_id}/{TID}/ -mp msit_dump/tensors/{device_id}_{PID}/{TID}/ --op-mapping-file xx/xxx/xx/`
...
* --my-path参数为第二步中 `atb_tensor`所在目录 `{DUMP_DIR}/msit_dump/tensors/{device_id}_{PID}/{TID}/`
