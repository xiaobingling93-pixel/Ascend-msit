...
| -er，--execute-range           | 指定dump的token轮次范围，区间左右全闭，可以支持多个区间序列，默认为第0次，使用方式：-er 1,3 或 -er 3,5,7,7（代表区间[3,5],[7,7],也就是第3，4，5，7次token）。此外，请确保输入多区间时的总输入长度不超过500个字符。 | 否   |
...
- 加速库模型与torch模型之间的自动比对，具体使用请参考[使用指导](/msit/docs/llm/工具-大模型精度比对.md)；
...
| 参数名       | 描述                                                         | 是否必选 |
| ------------ | ------------------------------------------------------------ | -------- |
| --exec       | 执行模型推理的命令。**注：用户需自行保证执行命令的安全性，并承担因输入不当而导致的任何安全风险或损失**                                           | 是       |
