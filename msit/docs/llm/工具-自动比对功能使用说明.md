...
| --my-path, -mp         | 待比较的数据路径，支持单个数据文件路径或文件夹                         | 必选 
| --weight, -w         | 指定是否需要输出大模型量化后权重和浮点权重的比较csv文件，不指定则不输出比较文件                         | 可选       |
...
| --stats, -st           | dump统计量比对，指定是否进行统计量比对并输出结果                   | 可选     |
