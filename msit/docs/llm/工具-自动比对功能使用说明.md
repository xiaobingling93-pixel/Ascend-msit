...
| -mp, --my-path         | 待比较的数据路径，为单个数据文件路径                         | 是 
| -w, --weight         | 指定是否需要输出大模型反量化后权重和浮点权重的比较csv文件，不指定则不输出比较文件                         | 否       |
| -mf, --op-mapping-file | 算子类型映射关系文件路径，加速库模型与torch模型比对场景下按需提供 | 否       |
| -l, --log-level        | 日志级别，默认为info，可选值有：debug, info, warning, error, fatal, critical | 否       |
| -o, --output           | 比对结果csv的输出路径                                        | 否       |
...
-mp：量化后权重路径，当前路径下需存在模型量化后权重.safetensors文件。
#### 输出csv文件参数说明：
