...
| -h, --help                    | 命令行参数帮助信息| 否 | 
| --set_random_seed                | 设定确定性计算的种子，默认为None表示不开启确定性计算。如果设置种子，那么需要输入一个能转换为int的值。使用示例：--set_random_seed 1                                                                                                                                                            | 否   |
| --enable-deterministic-algorithm                | 在开启确定性计算时，可选是否要启动torch的确定性算法模式，默认False，可选择开启为True。只有在设置 了srs的场景下才有效。使用示例：--enable-deterministic-algorithm True                                                                                                                                                                                                                             | 否   |
