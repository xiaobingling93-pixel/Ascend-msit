...
- 由于标杆数据和待比较数据的 dtype 可能不一致，为方便比较会将其全部都转换为 fp32 进行比较，如有转换操作，会有打屏信息。
...
整网比对场景是包含：CPU 或 GPU 与 NPU 环境落盘的整网数据比对，会落盘大模型中所有 layer、module、和算子的输入输出数据，更详细的整网比对流程可参考 [工具-大模型精度比对](./工具-大模型精度比对.md) 章节。
