...
* 一键式筛选问题数据，添加需要的评测指标，输入问题、回答、标杆答案，即可得到结果
*  工具只负责评测模型输出结果，不涉及模型性能分析
...
`CaseFilter` 是 `msit llm` 工具在此需求背景下，孕育出来的一个接口。它集成了不同的评测指标，使用起来也特别轻巧方便。目前还处于萌芽阶段，欢迎大家积极提供改进意见，帮助它成长。
