...
比对结束后会打印比对结果报告，罗列比对结果不同的服务化参数，并落盘为CSV文件。如果GPU和NPU或者NPU和NPU结果完全相同则打印No differences found。
...
### 2.1 模型初始化文件的比对
1. 下载模型权重文件夹，可在[huggingFace](https://huggingface.co/)，[gitee](https://gitee.com/hf-models), [魔塔社区](https://www.modelscope.cn/home)上下载。
