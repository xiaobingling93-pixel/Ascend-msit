...
* data_id 是数据匹配的唯一标识，需与加速库测数据匹配对应。
...
* `golden path` 为第二步中 `golden_tensor` 所在目录，即：`{DUMP_DIR}/ait_dump_{TIMESTAMP}/{PID}_{DEVICE_ID}/`
...
| --log-level, -l    | 日志级别，默认为 info                                                                      | 否       |
| --output, -o       | 比较结果 `.csv` 的输出路径，默认为当前路径                                                     | 否       |
