...
| random_seed | 设定启动确定性计算的种子 | 数据类型:int，可以通过该参数确定是否要启动确定性计算，输入的值表示固定随机性的种子值，默认为None表示不启动 | 否 | 7.0.0rc730 |
| enable_torch_deterministic | 设置是否需要启动torch的deterministic算法 | 数据类型:bool，可以通过该参数设定是否要在确定性计算的基础上进一步开启torch的确定性算法，默认False表示不开启。只有在random_seed不为None的时候才能工作 | 否 | 7.0.0rc730 |
