...
统计图的每种类型结点的数量，例如dump图的文件名为`ge_onnx_00449_graph_101_Build.pbtxt` [DUMP图描述信息](https://www.hiascend.com/document/detail/zh/canncommercial/80RC3/apiref/envvar/envref_07_0011.html) ，则查看图的统计信息的命令为：
...
显式出来的内容分别为结点类型和该类型结点在图上的数量，通过此命令，可以快速拿到一幅图的概要信息。
在确认两张图是否一致、查看融合规则是否生效等场景下，是一个较为快捷的确认方法。

## 抽取图

当图中节点数量非常多时，使用可视化工具分析图结构通常会卡顿，若只需要查看图中一部分，则可以按需抽取图的一小部分。

抽取子图的命令为`extract`，总体来说，抽取子图的方法分为两类，基于节点名的扩散抽取，以及基于起始和结束结点的区间抽取。
...
可以通过指定一个中心节点名的方式，并基于结点向前或向后做扩散，做子图的抽取，
例如图的文件名为`ge_onnx_00449_graph_101_Build.pbtxt`，以结点`MatMul_1/v2`为中心，想前后各扩散3层，执行命令：
...
2. 从图上查找名称为`MatMul_1/v2`的结点
3. 以结点`MatMul_1/v2`为中心，想输入方向、输出方向，循环查找所有的输入、输出结点，找3层为止，将所有找到的结点去重、保存下来
4. 将所有找到的结点dump成图
5. 查找所有找到的结点的输入输出结点，同样将其dump到图中

执行完成后，在`ge_onnx_00449_graph_101_Build.pbtxt`同级目录，会生成一个`ge_onnx_00449_graph_101_Build_*.pbtxt`的文件，此文件即为抽取出的子图。

关于抽取规则，通过下图的举例做详细说明。下图中(抽取示意图)，指定了蓝色为需要抽取的结点：

![抽取示意图](image/extract.PNG)

绿色部分是第3步中，向输入、输出扩散3层找到的所有结点。
蓝色、绿色可以认为是我们需要抽取的图的“骨干”，黄色部分则是这些骨干结点的叶子结点，也会被一起抽取下来。
灰色结点不属于骨干结点，也没有与骨干结点直接相连，则不会被抽取下来。

值得注意的是黄色的结点A，此节点距离蓝色结点的距离也是3，但此节点却不属于骨干上的结点。
原因是结点A是“输出方向分支上的一个输入结点“，而循环查找时，输出方向的骨干结点只会继续向输出方向查找，所以A不在骨干结点中。
但由于A直连了骨干结点（结点2），因此A仍然会出现在抽取图中。

#### 取消dump叶子结点

在[默认方式](#默认方式)中提到的，叶子结点会跟随其直连的骨干结点一起被dump到抽取的图中。
如果不希望此行为发生，可以通过`--without-leaves`选项来杜绝叶子结点被dump到抽取图中。
从[抽取示意图](#抽取示意图)来看，指定了该选项后，只有蓝色和绿色结点会被抽取出来。
...
此时可以通过`--stop-name`指定中止抽取，例如`--stop-name "allreduce"`，可指定停止的结点名字或类型。

#### 单向抽取

如果只希望从某个结点开始，只向前或向后做单向抽取，可以通过指定`--only-forward`或`--only-backward`指定，forward和backward的方向为：

![forward_backward](image/fw_bw.PNG)

### 区间抽取的方法
该方法需指定一组起始结点和终止结点，dump出起始到终止结点中间的所有结点。
