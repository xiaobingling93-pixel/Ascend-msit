...
| --dym-shape-range        | 动态Shape的阈值范围。如果设置该参数，那么将根据参数中所有的Shape列表进行依次推理，得到汇总推理信息。<br/>配置格式为：name1:1,3,200\~224,224-230;name2:1,300。其中，name为模型输入名，“\~”表示范围，“-”表示某一位的取值。<br/>直接配置阈值范围时，仅支持配置一组范围，多个输入之间用英文分号隔开，一组输入之间需要用引号括起来，否则会识别失败。<br/>例如：--dym-shape-range "input1:[8~20,3,5,-1];input2:[5,3~9,10,-1]"。<br/>也可以指定动态Shape的阈值范围配置文件*.info，该文件中记录动态Shape的阈值范围。指定info配置文件时，文件内可配置多组阈值范围，每组范围占一行且不需要添加引号（每组之间不能使用多余换行，会报错），例如：<br/>x1:1,3,32,32;x2:1~4,3,32,32<br/>x1:1~4,3,32,32;x2:1,3,32,32                                                                                                                                  | 否       |
