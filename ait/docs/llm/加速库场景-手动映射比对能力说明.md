...
* 在模型py文件中文件开头导入dump_data函数
  ...
  `from ait_llm.dump.manual_dump import dump_data`
...
`ait llm compare --golden-path {DUMP_DIR}/ait_dump_{TIMESTAMP}/{PID}_{DEVICE_ID}/ --my-path my-path.bin --output output_dir`
