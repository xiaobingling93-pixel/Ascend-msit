...
> - layer 信息落盘位置：在`{DUMP_DIR}/ait_dump_{TIMESTAMP}/layer/{PID}/`目录下，可以通过 layer 信息，获取到最大的编号，编号最大的既是模型最后一层
...
```shell
ait llm compare -gp {DUMP_DIR}/{cuda|cpu}{device_id}_{PID}/ -mp ait_dump_{TIMESTAMP}/tensors/{device_id}_{PID}/ -cl token
```
...
> - csv 报告查看参考[精度比对结果参数说明](/ait/docs/llm/精度比对结果参数说明.md)
