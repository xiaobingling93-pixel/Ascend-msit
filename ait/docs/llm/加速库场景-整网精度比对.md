...
`ait llm dump --exec "bash run.sh patches/models/modeling_xxx.py" --type tensor model -er 1,1`
...
> - 更多参数，可以参考[加速库 DUMP 功能使用说明](/ait/docs/llm/工具-DUMP加速库数据使用说明.md)
...
```shell
ait llm compare -gp torch_dump/{PID}_npu{device_id}/{TID}/ -mp ait_dump_{TIMESTAMP}/tensors/{device_id}_{PID}/{TID}/ --op-mapping-file xx/xxx/xx/
```
