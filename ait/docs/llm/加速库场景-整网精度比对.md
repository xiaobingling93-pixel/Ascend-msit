...
`msit llm dump --exec "bash run.sh patches/models/modeling_xxx.py" --type tensor model -er 1,1`

> - `--exec`参数是指定拉起执行大模型推理脚本的命令
> - `--type tensor model`参数是指定 dump 模型的 tensor 及 model 拓扑信息
> - `--er 1,1`指定 dump 的 token 轮次范围。
>   - 在 mindie RC1 版本之后，运行 atb 模型一般会先进行 warmup。导致第 0 个 token 轮次是 warmup 的数据，可以通过 er 参数指定 dump 第一个轮次解决该问题
>   - 在 mindie RC1 版本之后，atb 模型一般会有 prefile 和 decode 两个模型。prefill 只运行一次，所以其实在目录 1 中，会存在一次 prefill 推理 dump 数据和 decode 推理 dump 数据，会存储 2 个 token 的数据。后面编号的目录仅会存在 decode 一个 token 的 dump 数据
...
```shell
msit llm compare -gp torch_dump/{PID}_npu{device_id}/{TID}/ -mp msit_dump_{TIMESTAMP}/tensors/{device_id}_{PID}/{TID}/ --op-mapping-file xx/xxx/xx/
```

msit llm compare 提供有精度问题的数据与标杆数据之间的比对能力。

> - --golden-path 参数为第一步中 `torch_tensor`所在目录 `{DUMP_DIR}/{PID}_npu{device_id}/{TID}/`
> - --my-path 参数为第二步中 `atb_tensor`所在目录 `{DUMP_DIR}/msit_dump_{TIMESTAMP}/tensors/{device_id}_{PID}/{TID}/`
