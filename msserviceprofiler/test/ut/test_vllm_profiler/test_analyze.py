...
# Import fake profiler before test subject
...
# Setup environment for VLLM v1 hooks
os.environ['VLLM_USE_V1'] = '-1'

# Import test subject after mocks
...
async def test_add_request_async_given_request_id_and_prompt_when_called_then_events_logged_and_original_called():
    ...
    # Execute
    result = await request_hookers.add_request_async(
        original_func, mock_this, request_id, prompt
    )
...
def test_process_outputs_given_empty_outputs_when_called_then_no_events_and_original_called():
    ...
    # Execute
    result = request_hookers.process_outputs(
        original_func, mock_this, []
    )
...
def test_process_outputs_given_finished_request_when_called_then_httpRes_logged_with_metrics():
    ...
    # Execute
    result = request_hookers.process_outputs(
        original_func, mock_this, [mock_output]
    )
...
def test_process_outputs_given_missing_stats_when_called_then_httpRes_logged_with_none_reply_tokens():
    ...
    assert metrics == [
        ('recvTokenSize', 3),
        ('replyTokenSize', None)  # Missing stats
    ]
...
def test_process_outputs_given_multiple_requests_when_called_then_correct_events_logged():
    ...
    mock_this = MagicMock(request_states={
        "req_finished1": state1,
        "req_finished2": state2
    })
...
def test_process_outputs_given_original_function_arguments_when_called_then_arguments_passed_correctly():
    ...
    # Execute
    result = request_hookers.process_outputs(
        original_func, mock_this, [mock_output], *extra_args, **extra_kwargs
    )
    
    # Verify
    assert result == "test_result"
    original_func.assert_called_once_with(
        mock_this, [mock_output], *extra_args, **extra_kwargs
    )