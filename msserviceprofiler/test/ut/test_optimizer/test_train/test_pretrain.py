...
class TestPretrainModel(unittest.TestCase):
    ...
    def test_data_analysis(self):
        """测试数据分析功能"""
        # 准备分析目录
        analysis_dir = self.test_dir / "data_analysis"
        analysis_dir.mkdir()

        # 执行数据分析
        analysis_data(
            lines_data=self.real_data,
            save_path=analysis_dir,
            predict_field="model_execute_time"
        )

        # 验证特征分布图
        for plot_name in ["numerical", "categorical", "input_velocity"]:
            plot_file = analysis_dir / f"{plot_name}_distribution.png"
            self.assertTrue(plot_file.exists())

        # 验证相关性热图
        corr_file = analysis_dir / "correlation_heatmap.png"
    ...
    def test_pretrain_workflow(self):
        ...
        self.assertTrue((self.output_path / "ohe").exists())
        ...
        # 验证README存在
        self.assertTrue((self.output_path / "README.md").exists())
