...
class TestPSOOptimizer:
    ...
    def test_dimensions_all_fields_equal(self, optimizer):
        optimizer.target_field = [MagicMock(min=1, max=1),
                                  MagicMock(min=2, max=2),
                                  MagicMock(min=3, max=3)]
        assert optimizer.dimensions() == 0

    def test_dimensions_some_fields_different(self, optimizer):
        optimizer.target_field = [
            MagicMock(min=1, max=1),
            MagicMock(min=2, max=3),
            MagicMock(min=4, max=4)
        ]
        assert optimizer.dimensions() == 1

    def test_dimensions_empty_fields(self, optimizer):
        optimizer.target_field = []
        assert optimizer.dimensions() == 0
...
def test_best_params(generate_store):
    ...
    for index, row in optimizer_result.iterrows():
