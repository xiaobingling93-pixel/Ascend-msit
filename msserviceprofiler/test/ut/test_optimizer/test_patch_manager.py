...
class TestPatchManager(unittest.TestCase):
    ...
    def test_patch_already_applied(self):
        ...
        with patch("msserviceprofiler.modelevalstate.patch.patch_manager._patch_dir", self.temp_dir), \
            ...
            # 验证存在提示
            mock_info.assert_called_with("The patch aleady exists.")
    ...
    def test_patch_new_application(self):
        ...
        with patch("msserviceprofiler.modelevalstate.patch.patch_manager._patch_dir", self.temp_dir), \
            ...
            self.assertNotEqual(new_content, original_content, "新补丁应用后文件内容应被修改")

            # 验证补丁内容被添加到文件中
            self.assertIn("print('patch applied')", new_content, "补丁内容应被添加到文件中")

            # 验证成功日志
            mock_info.assert_called_with("The patch is installed successfully.")
