...
def test_num_mem_size_checker_given_valid_config_when_npu_mem_size_not_minus1_then_adds_answer():
    ...
    with patch.object(base_analyze.logger, 'info') as mock_log:
        base_analyze.num_mem_size_checker(
            test_config, 
            {},  # benchmark_instance
            "",  # mindie_server_log_path
            {}   # profiling_params
        )
        
        mock_log.assert_called_with("获取目前 numMemSize 的值为 1024, 并不是 -1")
        assert ("set to -1", "设置为-1，将由服务化自动根据剩余的显存数量，配置block数量，会尽量用满显存空间") in \
               ANSWERS[SUGGESTION_TYPES.config]["npuMemSize"]

def test_num_mem_size_checker_given_npu_mem_size_minus1_when_called_then_no_action():
    test_config = {
        "BackendConfig": {
            "ModelDeployConfig": {
                "ModelConfig": [{"npuMemSize": -1}]
            }
        }
    }
...
def test_check_prefill_latency_given_first_token_target_and_support_select_batch_true_then_recommends_disable():
    ...
    with patch.object(base_analyze.logger, 'debug') as mock_debug:
        ...
        assert ("set to False", "关闭 supportSelectBatch 可降低首 token 时延") in \
               ANSWERS[SUGGESTION_TYPES.config]["support_select_batch"]
