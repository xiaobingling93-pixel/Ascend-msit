...
def schedule(original_func, this, *args, **kwargs):
    ...
    for scheduled_new_req in scheduler_output.scheduled_new_reqs:
        prof.res(scheduled_new_req.request_id)
        state.running.add(state.waiting.pop(scheduled_new_req.request_id))
    ...
    for scheduled_cached_req in scheduler_output.scheduled_cached_reqs:
        if scheduled_cached_req.request_id not in state.running and scheduled_cached_req.request_id in state.waiting:
            prof.res(scheduled_cached_req.request_id)
            state.running.add(state.waiting.pop(scheduled_new_req.request_id))
            prof.metric_inc("RUNNING", 1).metric_inc("WAITING", -1).event("ReqState")
    ...
    for request_id in state.running:
        if request_id in this.waiting:
            prof.res(scheduled_cached_req.request_id)
            state.waiting.add(state.running.pop(scheduled_new_req.request_id))
            prof.metric_inc("RUNNING", -1).metric_inc("WAITING", 1).event("ReqState") 
