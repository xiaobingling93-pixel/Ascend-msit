...
### 修改配置文件
启动寻优前需用户按照实际情况配置`config.toml`（msserviceprofiler/msserviceprofiler/modelevalstate/config.toml），包括：
...
## 多机部署
多机部署（目前只支持双机部署）时，需要用户在两台机器上创建一个共享目录并在两台机器中的`config.toml`中添加`communication`字段，如：
```
n_particles = 10 
iters = 5 
ttft_penalty = 1 
