...
| 参数名 | 输入/返回值 | 含义 | 使用限制 |
| ------ | ------ | ------ | ------ |
| cache_ratio | 输入 | 加速比 | 必选。<br>数据类型：float。<br>范围：(1.0, 2.0) |
| dit_block_num | 输入 | DiT块数量 | 可选。<br>数据类型：int。<br>默认值：None |
| num_sampling_steps | 输入 | 采样步数 | 必选。<br>数据类型：int。<br>必须为正整数 |
...
1. cache_ratio控制缓存应用的加速比，值越大表示期望的加速效果越明显
2. dit_block_num通常由系统自动设置，无需手动指定
3. num_sampling_steps应与实际推理时的采样步数一致
