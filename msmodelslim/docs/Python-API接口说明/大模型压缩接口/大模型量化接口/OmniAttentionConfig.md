...
|参数名|含义|是否必填| 使用说明                                                                   |
|----|----|----|------------------------------------------------------------------------|
|pool_size|控制遗传算法初始化个体数量|非必填| 数据类型为INT，数值范围>0，默认值为50，建议使用默认值（搜寻时间在10h左右）。个体数量越大，耗时越久                 |
|num_mutation|每轮进化变异的个体数量|非必填| 数据类型为INT，数值范围>0，默认值为10，建议使用默认值。仅在调用search_incremental()方法中使用 |
