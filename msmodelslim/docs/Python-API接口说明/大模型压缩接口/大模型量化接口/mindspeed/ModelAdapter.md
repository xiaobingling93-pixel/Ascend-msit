...
### 函数原型
```python
ModelAdapter(model:nn.Module, dev_type='npu', forward_step=None)
```
...
### 函数原型
```python
ModelAdapter.set_prefix(prefix)
```

### 参数说明
| 参数名| 输入/返回值 | 含义 | 使用限制 |
| ------ | ------ | ------ | ------ |
| prefix | 输入 | 模型。| 必选。<br>当模型的state_dict返回的keys和模型本身结构不一致时，必须通过调整prefix来保证一致性。非必须调用接口|

### 调用示例
```python
from msmodelslim.pytorch.mindspeed import ModelAdapter
model = ModelAdapter(model)
# 可选，当state_dict与model的结构不一致时配置
model.set_prefix('model.module.')
```
