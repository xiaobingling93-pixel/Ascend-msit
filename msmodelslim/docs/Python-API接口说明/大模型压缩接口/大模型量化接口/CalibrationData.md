...
```python
# 添加用户自定义数据集接口，需在set_sample_size(sample_size)之前调用
...
#               dataset_name需为config.json文件中配置的数据集名称，或为用户自定义数据集名称，大小写敏感；
#               size数据类型为 INT，若为空或0值，则返回混合校准集对应数据集的采样数量为0；若为非INT，则报错
