...
def get_calib_dataset(tokenizer, calib_list, device=f"npu:{device_id}"):
    ...
    return calib_dataset
```
注： 需要将msmodelslim文件夹下的[precision_tool文件夹](../precision_tool/precision_tool.py)复制一份出来，和量化脚本放置于同一目录下，再将待测试数据集放入precision_tool文件夹中，具体操作见：[Precision Tool 使用方法说明及数据集下载链接](https://gitee.com/ascend/msit/tree/master/msmodelslim/precision_tool)  
