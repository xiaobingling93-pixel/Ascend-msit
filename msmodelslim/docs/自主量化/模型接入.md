...
1. **新建模型适配器`py`文件**：建议放在[`msmodelslim/model/`](../../msmodelslim/model/qwen3) 下，命名如 `qwen3.py`。
2. **理清量化过程涉及的组件，以组件接口组合定义适配器类**：模型适配器类必须继承自[`BaseModelAdapter`](../../msmodelslim/model/base.py)。
...
| 算法               | 算法介绍       | 适配指导       |
| ------------------ | ------------- | ------------- |
| SmoothQuant | [SmoothQuant：离群值抑制算法说明](../算法说明/Smooth_Quant.md) | [SmoothQuant 适配](../算法说明/Smooth_Quant.md#模型适配)
| Iterative Smooth | [Iterative Smooth：离群值抑制算法说明](../算法说明/Iterative_Smooth.md) | [Iterative Smooth 适配](../算法说明/Iterative_Smooth.md#模型适配)
...
| Flex AWQ SSZ | [Flex AWQ SSZ：灵活激活感知权重量化平滑算法说明](../算法说明/Flex_AWQ_SSZ.md#flex-awq-ssz灵活激活感知权重量化平滑算法说明) | [Flex AWQ SSZ 适配](../算法说明/Flex_AWQ_SSZ.md#模型适配)
