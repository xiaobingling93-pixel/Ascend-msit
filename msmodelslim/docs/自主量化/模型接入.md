...
下文以 [`Qwen3-32B`](../../msmodelslim/model/qwen3.py) W8A8动态量化场景（简称“场景示例”）的模型接入为例
...
2. **理清量化过程涉及的组件，以组件接口组合定义适配器类**：模型适配器类必须继承自[
   ...
   。在场景示例中由于经验上W8A8动态量化精度损失很小，无需搭配离群值抑制算法，也很少需要回退，我们仅需要支持量化调度，不需要支持离群值量化、敏感层分析等额外功能
...
4. **注册模型名**：在模型适配器上注册模型名，以便同系列模型复用一个模型适配器
...
5. **注册模型适配器**：在 [`model/__init__.py`](../../msmodelslim/model/__init__.py) 注册模型适配器，以便msModelSlim感知和使用新的模型适配器
...
2. **量化自有模型**：可通过如下命令完成自有模型量化，请注意`trust_remote_code`为`True`时可能执行浮点模型权重中代码文件，请确保浮点模型来源安全可靠

```bash
msmodelslim quant --model_path {浮点模型路径} \
                  --save_path {目标保存路径} \
                  --device npu \
                  --model_type {注册的模型名} \
