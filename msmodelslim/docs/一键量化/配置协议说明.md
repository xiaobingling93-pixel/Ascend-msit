...
spec:
  process:
    - type: "iter_smooth"
      ...
      symmetric: False                     # 是否启用对称量化，默认False。
...
| symmetric | 是否对称量化  | 布尔值，True为对称，False为非对称，默认False。 |
...
| 字段名 | 作用 | 说明 |
|--------|------|------|
| type | 处理器类型标识 | 固定值"dynamic_cache"，用于标识该对象为 KVCache 量化处理器。 |
| qconfig | KVCache量化配置 | 包含KVCache的量化参数配置。 |
| include | 包含的注意力层模式 | 支持通配符匹配，指定要执行KVCache量化的注意力层。 |
| exclude | 排除的注意力层模式 | 支持通配符匹配，优先级高于include。 |
...
暂不支持配置pipeline流水线类型。当前多模态生成模型考虑到显存占用问题，默认且仅支持layer_wise逐层量化形式。
...
| task | 任务类型 | 指定模型任务类型 | "t2v-14B", "t2v-1.3B" |
| size | 生成尺寸 | 视频或图像的尺寸规格 | "1280\*720", "832\*480" |
| sample_steps | 采样步数 | 扩散模型的采样步数 | 大于0的整数 |

**任务类型说明**:

- **t2v-14B**: 文本到视频任务，14B模型
- **t2v-1.3B**: 文本到视频任务，1.3B模型
