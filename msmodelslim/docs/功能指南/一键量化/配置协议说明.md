...
**详细文档**: [Iterative_Smooth算法说明.md](./v1架构/算法/Iterative_Smooth算法说明.md)
...
**详细文档**: [kv_smooth.md](./v1架构/算法/kv_smooth.md)
...
**详细文档**: [Flex_Smooth_Quant算法说明.md](./v1架构/算法/Flex_Smooth_Quant算法说明.md)
...
**详细文档**: [KVCache_quant.md](./v1架构/算法/KVCache_quant.md)
...
**作用**: 通过 ADMM（Alternating Direction Method of Multipliers，交替方向乘子法）算法实现模型浮点稀疏化，结合L2量化保持重要位置的精度，在保证模型性能的同时实现高压缩率。

**详细文档**: [float_sparse.md](./v1架构/算法/float_sparse.md)
...
**作用**: 使用旋转矩阵对模型进行异常值抑制处理，通过对权重和激活值进行特定的旋转变换，将异常值“分散”到多个通道中，从而在量化前显著平滑数据的分布，有效降低量化误差。

**详细文档**: [QuaRot.md](../一键量化/v1架构/算法/QuaRot.md)
...
| max_tp_size             | 最大张量并行大小         | int       | 4      | 最大张量并行大小，仅在启用在线旋转时生效，必须大于0且为2的幂，拉起模型时设置的tp值必须<=max_tp_size。                           |
| down_proj_online_layers | 指定使用在线旋转的down层 | list[int] | []     | 用于指定哪些层的down_proj使用在线旋转，类型为由层索引组成的列表，默认为空。                             |

###### autoround_quant

**作用**: 通过引入可学习的舍入偏移参数，结合SignSGD优化器自适应调整各权重的舍入方向，有效降低量化重构误差，实现低比特量化。

**详细文档**: [AutoRound.md](../一键量化/v1架构/算法/AutoRound.md)
...
default_w8a8_dynamic: &default_w8a8_dynamic
  weight:
    ...
    ext:
      ...
      scale_dtype: "bfloat16" # 缩放因子数据类型：bfloat16
  act:
...
| strategies           | 量化策略               | list     | -      | 用于指定量化策略，支持混合量化策略。用于指定量化策略，支持int4和int8混合量化策略，量化粒度支持权重per_group和per-channel、激活per_token。                         |
