...
W8A16量化精度调优策略是结合modelslim量化工具和精度测试工具precision tool进行精度验证和调优开展。因为w8a16本身量化后精度就很高，大部分模型量化后都精度能满足需求，所以就不结合具体的案例展开精度调试过程，如果发现有W8A16量化后精度不合格，可以参考本文的精度调优策略进行调试。
另外，W4A16跟W8A16一样，都是仅权重量化，因此，精度调优策略相同。
...
model = AutoModelForCausalLM.from_pretrained(
    pretrained_model_name_or_path=fp16_path).half().to(f"npu:{device_id}") 
...
anti_outlier.process()
```
补充：其中anti_method有五种算法，m3是用于权重量化的，另外四种是用于激活值量化的场景，如果是激活值精度调优建议从m1到m5逐一尝试。
...
增加回退层（建议最后进行调整），可以按照一定的经验，通过disable_name手动设置。
...
def get_calib_dataset(tokenizer, calib_list, device=f"npu:{device_id}"):
    ...
    return calib_dataset
```
注： [Precision Tool 使用方法说明及数据集下载链接](https://gitee.com/ascend/msit/tree/master/msmodelslim/precision_tool)  

# 4 量化回退（回退的都是Linear层）

大模型量化是一种压缩技术，它通过压缩模型权重和激活数值的表示精度来降低模型的存储和计算需求。量化工具通常将高位浮点数转化为低位定点数，例如32bit降低至8bit，从而直接减少模型权重的大小，在无需训练成本的前提下，即可完成大模型的训练后压缩并最大程度保障其精度。仅权重量化操作流程：
1.判断需要回退的层，通过看日志中range_parm的数值,数值越大表明越需要回退。比如down_proj层，o_pro层。
2.操作方式：QuantConfig里设置参数，disable_names设置需要回退的量化敏感层。

另外，W8A16仅支持手动回退，不能使用自动回退方式，即设置disable_name=0。
