...
| mindie_format | 是否开启旧的权重配置文件保存格式 | False | 指定`mindie_format=True`保存仅适配MindIE的权重配置文件保存格式。|
...
- 生成Qwen3-30B-A3B模型 w8a8 混合量化权重
  ```shell
  python3 quant_qwen_moe_w8a8.py --model_path {浮点权重路径} --save_path {W8A8量化权重路径}
  ```
