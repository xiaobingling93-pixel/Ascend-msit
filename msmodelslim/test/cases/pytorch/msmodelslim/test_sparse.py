...
def depth_model():
    yield nn.Sequential(
        ...
        nn.nn.Sequential(nn.Conv2d(32, 64, 1, 1, bias=False), nn.Conv2d(64, 32, 1, 1, bias=False)),
        nn.nn.Sequential(nn.Conv2d(32, 64, 1, 1, bias=False), nn.Conv2d(64, 32, 1, 1, bias=False)),
        nn.nn.Sequential(nn.Conv2d(32, 64, 1, 1, bias=False), nn.Conv2d(64, 32, 1, 1, bias=False)),
        nn.nn.Sequential(nn.Conv2d(32, 64, 1, 1, bias=False), nn.Conv2d(64, 32, 1, 1, bias=False)),
        nn.nn.Sequential(nn.Conv2d(32, 64, 1, 1, bias=False), nn.Conv2d(64, 32, 1, 1, bias=False)),
...
def test_sparse_model_width_given_valid_when_eval_then_pass(width_model, width_optimizer):
    ...
    oring_model_params = count_parameters(width_model)
...
def test_sparse_model_width_given_invalid_optimizer_when_any_then_error(width_model, width_optimizer):
    with pytest.raises(TypeError):
...
def test_sparse_model_width_given_invalid_epochs_each_stage_when_any_then_error(width_model, width_optimizer):
    with pytest.raises(TypeError):
        sparse.sparse_model_width(width_model, width_optimizer, steps_per_epoch=2, epochs_each_stage=[1])
    
    with pytest.raises(TypeError):
        sparse.sparse_model_width(width_model, width_optimizer, steps_per_epoch=2, epochs_each_stage=[-1, 1])


def test_sparse_model_depth_given_valid_when_eval_then_pass(depth_model, depth_optimizer):
    steps_per_epoch, epochs_each_stage = 2, [2, 2, 1]
    oring_model_params = count_parameters(depth_model)
