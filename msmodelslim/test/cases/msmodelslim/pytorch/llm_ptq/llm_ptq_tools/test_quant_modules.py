...
class TestQuantizer:
    ...
    def test_input_quantizer_tensor_forward_should_return_expected_result(self):
        # 普通a_bit=8场景的激活的Quantizer.tensor_tensor测试
        cfg = QuantConfig().weight_activation_quant()
        test_quantizer = Quantizer(
            bit=8, is_signed=True, is_enable=True, is_input=True, cfg=cfg, logger=logger, is_dynamic=False
        )
        ...
        result = test_quantizer.tensor_tensor(in_param_data)
    ...
    def test_input_quantizer_tensor_forward_with_dynamic_quant_should_return_expected_result(self):
        # 普通a_bit=8动态量化场景的激活的Quantizer.tensor_tensor测试
        cfg = QuantConfig().weight_activation_quant()
        test_quantizer = Quantizer(
    ...
    def test_input_quantizer_tensor_forward_with_int_infer_should_return_expected_result(self):
        ...
        _ = test_quantizer.tensor_tensor(in_param_data)
    ...
    def test_weight_quantizer_tensor_forward_should_return_expected_result(self):
        # 普通w_bit=8场景的权重的Quantizer.tensor_tensor测试
        cfg = QuantConfig().weight_activation_quant()
        test_quantizer = Quantizer(
    ...
    def test_weight_quantizer_tensor_forward_with_int_infer_should_return_expected_result(self):
        # 普通的w_bit=8量化场景的激活Quantizer.tensor_forward int infer测试
        cfg = QuantConfig().weight_activation_quant()
        test_quantizer = Quantizer(
            bit=8, is_signed=True, is_enable=True, is_input=False, cfg=cfg, logger=logger, is_dynamic=False
        )
    ...
    def test_init_weight_quant_normal_should_return_expected_result(self):
        # 普通的w_bit=8场景的权重Quantizer._init_weight_quant_normal测试
        cfg = QuantConfig().weight_activation_quant()
        test_quantizer = Quantizer(
            bit=8, is_signed=True, is_enable=True, is_input=True, cfg=cfg, logger=logger, is_dynamic=False
        )
    ...
    def test_init_weight_quant_normal_with_hessian_should_return_expected_result(self):
        ...
        cfg = QuantConfig(w_bit=8, a_bit=8).weight_quant(w_method="GPTQ")
        test_quantizer = Quantizer(
            bit=8, is_signed=True, is_enable=True, is_input=False, cfg=cfg, logger=logger, is_dynamic=False
        )
        in_param_weight = torch.tensor([1.0, 1.0, 1.0, 1.0])
