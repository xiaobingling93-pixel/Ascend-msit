...
@logger_setter('msmodelslim.app.analysis_service.layer_selector')
class LayerSelectorAnalysisService(BaseAnalysisService):
    ...
    def analyze(self, 
        ...
        if not isinstance(model, BaseModelAdapter):
            raise SchemaValidateError("model must be a BaseModelAdapter",
                                      action='Please ensure the model is a BaseModelAdapter instance')
        ...
        if model.device.value == 'npu':
            torch.npu.set_compile_mode(jit_compile=False)
        
        # Prepare calibration data
        calib_data = self._prepare_calibration_data(model, calib_dataset_name)
    ...
    def _prepare_calibration_data(self, model: BaseModelAdapter, calib_dataset_name: Optional[str]):
