...
class PrecisionTest:
    def __init__(self, model, tokenizer, dataset, batch_size, hardware_type,
        ...
        """
        self.logger = logger
        self.logger.info("Ready to init precision test tool.")
        ...
        if not isinstance(self.tokenizer_return_type_id, bool):
            raise TypeError("Tokenizer return_type_id must be bool.")
        self.tokenizer = tokenizer
        if not self.__verify_tokenizer():
            raise TypeError("Tokenizer must matched with Model.")
    ...
    def __verify_batch_size(self):
        ...
        if not self.batch_size > 0:
            self.logger.error("Batch size must be more than 0.")
            return False
        return True
    ...
    def __verify_tokenizer(self):
        ...
        try:
            ...
            self.logger.error(f"Model and tokenizer are not compatible. Cannot simply use model API."
                              f"Type of model is {type(self.model)} while type of tokenizer is {type(self.tokenizer)}")
            return False
        try:
            with torch.no_grad():
                self.model.generate(**inputs, do_sample=False, max_new_tokens=1)
        except Exception as e:
            self.logger.error(f"Model and tokenizer are not compatible. Cannot simply use generate API."
                              f"Type of model is {type(self.model)} while type of tokenizer is {type(self.tokenizer)}")
            return False
