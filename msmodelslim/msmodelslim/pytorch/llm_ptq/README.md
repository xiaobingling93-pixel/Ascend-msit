...
1. 调整量化参数。例如W8A8量化调整act_method，W8A16量化更换使用的w_method。act_method默认为1，可以选 ‘1’ ‘2’ ‘3’，LLM大模型场景下推荐使用3。（稀疏量化的情况下只支持1和2的方式）

2. 稀疏量化可以调整fraction，建议在0.01~0.1之间取值；也使用sigma调整sigma_factor，建议在3.0~4.0内取值；还可以将nonuniform参数设置为True。Lowbit场景下，除了上述微调方法以外，还可以将use_sigma参数设置为True进行精度微调。
