    ...
    def modify_config(model_dir, dest_dir, torch_dtype, quantize_type, **kwargs):
        ...
        if kwargs is not None:
            quantization_config = {
                ...
                'group_size': kwargs.get("group_size", 0) if kwargs.get("is_lowbit", False) and
                                                             not kwargs.get("open_outlier", True) else 0,
                'kv_quant_type': "C8" if kwargs.get("use_kvcache_quant", False) else None,
                "fa_quant_type": "FAQuant" if kwargs.get("use_fa_quant", False) else None,
                'w_bit': kwargs.get("w_bit", 8),
                'a_bit': kwargs.get("a_bit", 8),
                'dev_type': kwargs.get("dev_type", "cpu"),
                'fraction': kwargs.get("fraction", 0.01),
                'act_method': kwargs.get("act_method", 1),
                'co_sparse': kwargs.get("co_sparse", False),
                'anti_method': kwargs.get("anti_method", ""),
                'disable_level': kwargs.get("disable_level", "L0"),
                'do_smooth': kwargs.get("do_smooth", False),
                'use_sigma': kwargs.get("use_sigma", False),
                'sigma_factor': kwargs.get("sigma_factor", 3.0),
                'is_lowbit': kwargs.get("is_lowbit", False),
                'mm_tensor': kwargs.get("mm_tensor", False),
                'w_sym': kwargs.get("w_sym", True),
                'open_outlier': kwargs.get("open_outlier", True),
                'is_dynamic': kwargs.get("is_dynamic", False)
            }
            if hasattr(kwargs, 'pdmix') and kwargs.get("pdmix"):
                quantization_config.update({"pdmix": kwargs.get("pdmix")})
