...
# 预检
在部署 PD 分离场景和 PD 混部场景时，大量的配置文件需要用户手动修改，极易出错，切难以定位。预检工具支持 PD 混部和 PD 分离场景的各个配置文件的校验，对于异常的配置，工具通过终端提示错误内容，配置文件会直接展示需要关注的行号，提高部署效率。
...
另外，如果是多机混部 PD 场景，则需要在多台机器上 <strong>手动执行</strong> 预检命令，工具 <strong>不支持</strong> 一台机器控制多台机器执行。<br><br>
目前只针对于 DeepSeek 模型部署在 A2（默认双机）或者 A3 场景下，进行相关校验。
...
- `pd_dissegregation`: 多机 PD 分离场景
- `pd_dissegregation_single_container`: 单机 PD 分离场景
...
- `<rank-table-path>` 为 *rank table* 路径，用于触发 `hccn_tool` 相关的校验，以及多机之前的连通性测试，并且会对配置文件字段进行检查
...
```bash
$ msprechecker dump
Error occured while collecting 'hccl': 宿主机上没有找到 'hccn_tool' 命令
...
```bash
msprechecker compare baseline.json dumpoed.json
```
...
| 参数名         | 参数描述                                             | 是否必选                       |
| -------------- | ---------------------------------------------------- | ------------------------------ |
| --scene | 指定 PD 分离部署场景，支持单机 PD 分离 `pd_disaggregation` 和多机 PD 分离 `pd_disaggregation_single_container`。该参数需要和 `--config-parent-dir` 同时使用 | 否 |
