...
def show_legacy_warnings(args: argparse.Namespace):
    if getattr(args, "log_level", None):
        global_logger.warning(
            "DeprecationWarning: The '--log_level' argument is deprecated. Please use '--severity-level' instead.",
        )
    if getattr(args, "checkers", None):
        global_logger.warning(
            "DeprecationWarning: The '--checkers' argument is deprecated. HCCL will be checked whenever '--rank-table-path' is given, "
    ...
    if getattr(args, "service_config_path", None):
        global_logger.warning(
            "DeprecationWarning: The '--service_config_path' argument is deprecated. Please use '--mies-config-path' instead.",
        )
        args.mies_config_path = args.service_config_path
    if getattr(args, "mindie_env_config_path", None):
        global_logger.warning(
            "DeprecationWarning: The '--mindie_env_config_path' argument is deprecated. Please use '--mindie-env-path' instead.",
        )
        args.mindie_env_config_path = args.mies_config_path
    if getattr(args, "ranktable_file", None):
        global_logger.warning(
            "DeprecationWarning: The '--ranktable_file' argument is deprecated. Please use '--rank-table-path' instead.",
        )
        args.ranktable_file = args.rank_table_path
    if getattr(args, "sha256_blocknum", None):
        global_logger.warning(
            "DeprecationWarning: The '--sha256_blocknum' argument is deprecated. It is now migrated to 'msprechecker dump' option and "
            "use '--chunk-size' instead. No check will be applied. This API is maintained for backward compatibility.",
        )
        args.chunk_size = args.sha256_blocknum
    if getattr(args, "additional_checks_yaml", None):
        global_logger.warning(
            "DeprecationWarning: The '--additional_checks_yaml' argument is deprecated. Please use '--custom-config-path' instead.",
        )
        args.custom_config_path = args.additional_checks_yaml
    if getattr(args, "save_env", None):
        global_logger.warning(
            "DeprecationWarning: The '--save_env' argument is deprecated. Environment changes file will be stored under current working directory.",
        )
    if getattr(args, "dump_file_path", None):
        global_logger.warning(
            "DeprecationWarning: The '--dump_file_path' argument is deprecated. Please use '--output-path' instead.",
        )
